...
```javascript
// PptxGenJS 是全局构造函数（require('pptxgenjs') 也可）；writeFile 已接入沙箱
async function main() {
...
形状/图表枚举：`pres.ShapeType.rect`、`pres.shapes.RECTANGLE`、`pres.charts.BAR` 均可用。
...
slide.addImage({ data: imageData, x: centerX, y: 1.2, w: calcWidth, h: maxHeight });
```
...
**本运行时无 react-icons/sharp**。用「色块 + 符号文字」组合做图标效果：

```javascript
// 彩色圆 + 符号文字
function addIconBadge(slide, x, y, symbol, bgColor) {
  slide.addShape(pres.ShapeType.ellipse, {
    x, y, w: 0.5, h: 0.5, fill: { color: bgColor },
  });
  slide.addText(symbol, {
    x, y, w: 0.5, h: 0.5, fontSize: 18, color: "FFFFFF",
    align: "center", valign: "middle",
  });
}
addIconBadge(slide, 1, 1, "✓", "4472C4");
```

- 常用符号：`✓ ✗ ★ ▶ ● ◆ ➜ ⚠`，或主题相关 emoji（📊 💡 🎯 🏆）
- 若用户提供了图标 PNG 文件，按 Images 一节 base64 方式插入
...
// Image from base64（仅 data: 形式，无 URL/path 加载）
