...
系统 SHALL 将 Tavily API Key 以 provider `"tavily"` 存入现有 Secrets 存储（与模型 Key 相同机制），不以明文写入 SQLite 或 Agent 事件日志；界面与日志 MUST NOT 回显明文 Key。

#### Scenario: 保存 Tavily Key
- **WHEN** 用户在侧栏 Web 搜索区块输入 Tavily API Key 并保存
- **THEN** 系统调用 `set_api_key("tavily", ...)` 持久化，侧栏摘要变为「已启用」

#### Scenario: 清空 Tavily Key
- **WHEN** 用户清空已保存的 Tavily Key
- **THEN** Key 从 Secrets 移除，Web 搜索能力对该用户不可用
