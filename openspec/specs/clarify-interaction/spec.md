...
系统 SHALL 提供 `clarify_ask` 工具，供 Agent 在需求澄清流程中发起结构化问题。工具参数 MUST 符合 `ClarifyQuestion` schema（`id`、`kind`、`prompt` 必填；`kind` 为 `single` | `multi` | `text` | `confirm_brief`）。`single`/`multi` MUST 携带 2–6 个 `options`；`confirm_brief` MUST 携带 `brief` 字段；`allow_custom` 默认为 true。

#### Scenario: Agent 发起单选澄清题

- **WHEN** Agent 调用 `clarify_ask` 且 `kind=single`，含 2–6 个 `options`
- **THEN** 系统校验通过并进入 clarify 暂停流程（见 agent-loop spec），向前端 emit `clarify_question`

#### Scenario: 参数非法被拒绝

- **WHEN** Agent 调用 `clarify_ask` 但缺少 `id`、`kind`、`prompt`，或 `single` 无 `options`
