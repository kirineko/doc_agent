...
为 Chat 输入框提供静态斜杠命令注册表与模糊搜索，使用户可通过 `/` 快速填入面向 Agent 工具链的任务模板 prompt。
...
注册表 MUST 包含且仅包含 design 定义的 **22 条**命令；**阅读分析**能力 MUST 仅通过 id 为 `read` 的命令提供，不得为 word/ppt/pdf/excel 单独提供 read 类命令。
...
匹配结果 MUST 保持分类分组；分类顺序不变，组内按匹配得分降序。无匹配时 MUST 展示空状态提示；无匹配时 Enter MUST NOT 发送消息。

#### Scenario: 按类别过滤

- **WHEN** 用户输入 `/word`
- **THEN** 候选展示 category 为 word 的命令（如新建 Word、精准修改 Word）
