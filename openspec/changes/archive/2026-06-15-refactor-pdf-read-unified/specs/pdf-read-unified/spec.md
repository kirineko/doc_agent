...
当走全量 vision 时，行为 MUST 与现有实现一致：渲染（可缓存）→ 每批最多 4 页 vision → 合并 `markdown`；可附带 `text_layer` 与 `cache_hit`。

#### Scenario: 9 页全量 vision 分三批
...
`pdf_read` 返回 JSON MUST 包含 `resolved`（`text` | `vision`）与 `judge` 对象（含 `skipped`、`method`、`sample_page`、`sample_reason`、`verdict` 等适用字段），供 Agent 与验收使用。
