...
系统 SHALL 提供 `image_read` 工具，读取项目沙箱内图片文件并通过 vision 能力生成文本描述。参数 MUST 包含 `path`（项目相对路径）；可选 `prompt`（理解指令，默认「请详细描述图片内容」）。

工具执行 MUST 使用当前会话已锁定的 vision 模型发起**独立** `chat/completions` 请求（单轮、无 tools）：user `content` 为 `[image_url, text]`。返回结果 MUST 为纯文本 JSON（含 `text` 字段及可选 `mime`、`width`、`height`），供 Agent 后续推理。

#### Scenario: vision 模型读取 PNG

- **WHEN** 会话模型为 Kimi K2.6，Agent 调用 `image_read` 指向 `.uploads/a.png`
- **THEN** 工具返回非空文本描述，且主 Agent 循环的 tool 消息仅含该文本

#### Scenario: 自定义 prompt

- **WHEN** Agent 调用 `image_read` 且 `prompt` 为「提取图中所有公式」
