...
本版本改进模型服务失败时的提示与恢复，并加快含图片的 PPT 生成。

### 模型服务错误提示与自动重试（improve-provider-error-handling）

- **独立错误卡片**：鉴权失败、额度不足、网络中断等不再拼进回答正文，而是显示在消息下方的错误卡片；含处理建议，详情默认收起，可复制
- **自动重试**：限流、服务端瞬时错误、网络抖动会自动重试最多 2 次；重试时显示「网络波动，正在重试（1/2）…」
- **错误日志**：失败记录写入本机 `logs/provider-errors.jsonl`（路径见 README「数据存储」），便于排查
- **超时**：连接约 15 秒、流式空闲 90 秒；卡住时会提示超时，而不是一直转圈

### 含图片的 PPT 更稳更快（fix-skill-run-image-perf）

- **大图不再轻易超时**：读取、解码、缩放图片改走原生实现，多张照片插入 PPT 可在合理时间内完成
- **超时原因更清楚**：脚本超时会明确提示，工具链卡片可展开详情，而不是笼统的 JavaScript runtime error
...
本版本新增界面缩放配置，便于在高分辨率屏幕或长时间使用时放大整体 UI。

### 界面缩放（add-ui-scale）

- **设置抽屉**：新增「界面缩放」滑块，范围 100%–200%，步进 20%（100 / 120 / 140 / 160 / 180 / 200）
- **整页缩放**：通过 Tauri WebView `setZoom` 等比放大顶栏、三栏、Drawer 与 Markdown 等全部 UI
- **持久化**：偏好写入 `localStorage`（`doc-agent-ui-scale`），重启后自动恢复
- **快捷键**：⌘/Ctrl `+` 放大、`-` 缩小、`0` 恢复 100%（与滑块同步）
- **恢复默认布局**：「恢复默认布局」同时重置三栏比例与界面缩放至 100%
...
- **新工具 `image_download`**：批量下载 1–20 个公网 http(s) 图片到项目目录（默认 `images/`），返回本地相对路径与尺寸/格式元数据；制作 Word / PPT / PDF / HTML 前先落地图片，再按路径引用（`skill_run` 运行时无法联网取图）
- **与文档生成解耦**：独立工具，不依赖 Tavily 或任何 API Key；始终注册，不受 Web 搜索开关影响
- **安全与健壮**：仅 http/https；SSRF 尽力防护（私网/环回/链路本地、重定向目标复校验、跳数上限 10）；单图 15 MiB 流式上限；按真实字节校验图片类型（防 HTML 错误页伪装）；批量部分失败不中断
- **沙箱**：禁止 `dir` 为项目根或 `.cache/`；符号链接目标名与祖先目录逃逸防护；`create_new` 写盘
- **UI**：工具链中文标签「下载图片」；成功路径进入构建产物 Tab 与 `@` 文件索引
